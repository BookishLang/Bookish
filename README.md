...
You are also able to ask for help via discussions, submit any issues you encounter or any requests. This can also be considered the help/issues/feature request page of the **Bookish IDE**. If you're wondering why we only have an `.exe` for Windows, that's because the **IDE is supported on Windows only**, however you can probably use **Wine** for Linux.
